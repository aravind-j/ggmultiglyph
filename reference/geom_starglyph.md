...
(Siegel et al. 1972; Chambers et al. 1983; duToit et al. 1986) in a
scatterplot.
...
- draw.grid:
  ...
  logical. If `TRUE`, grid points are plotted along the whiskers if all
  the variables specified in `cols` are an ordered
...
`geom_starglyph()` understands the following aesthetics (required
