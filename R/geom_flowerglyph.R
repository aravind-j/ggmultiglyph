...
#' @inheritParams flowerglyphGrob
#' @param fill.petal The fill colour of the petals.
#' @param colour.petal The outline colour of the petals.
#' @param colour.centre The line colour of the central point.
#' @param fill.centre The fill colour of the central point.
#' @param colour.grid The colour of the grid lines.
#' @param linewidth The petal outline line width.
...
#'   central axis of each petal if all the variables specified in \code{cols}
#'   are an ordered \link[base]{factor}. Default is \code{FALSE}.
#'
#' @section Aesthetics: \code{geom_flowerglyph()} understands the following
...
geom_flowerglyph <-
  function(mapping = NULL, data = NULL, stat = "identity",
    ...
    params <- list(
      ...
      petal.base.shape = petal.base.shape,
      petal.tip.shape = petal.tip.shape,
      petal.tip.notch = petal.tip.notch,
      petal.tip.notch.width = petal.tip.notch.width,
      petal.base.notch = petal.base.notch,
      petal.base.notch.width = petal.base.notch.width,
      petal.waist = petal.waist,
      petal.waist.position = petal.waist.position,
      petal.waist.width = petal.waist.width,
      petal.curvature = petal.curvature,
      petal.curvature.position = petal.curvature.position,
      petal.curvature.width = petal.curvature.width,
      scale.length = scale.length,
      scale.area = scale.area,
      fill.petal = fill.petal,
      fill.gradient = fill.gradient,
      colour.petal = colour.petal,
      colour.grid = colour.grid,
      linewidth = linewidth,
      linewidth.grid = linewidth.grid,
      linejoin = linejoin,
      centre = centre,
      colour.centre = colour.centre,
      fill.centre = fill.centre,
    ...
    geomout <-
      ggplot2::ggproto(NULL, GeomFlowerGlyph,
                       required_aes = c(GeomFlowerGlyph$required_aes,
                                        cols),
                       default_aes = {
                         aes_new <- c(GeomFlowerGlyph$default_aes,
                                      legend.glyph.dims)
...
GeomFlowerGlyph <-
  ggplot2::ggproto(
    ...
    setup_data = function(data, params) {
      ...
      # Check if fill.petal are valid
      if (!is.null(params$fill.petal)) {
        if (length(params$fill.petal) != length(cols))
          stop('The number of colours specified in',
               '"fill.petal" are not equal to the number',
               'of variables specified in "cols".')

        if (!all(iscolour(params$fill.petal))) {
          stop('Invalid colour(s) specified in "fill.petal".')
        }

        data$fill <- NULL
      }

      # Check if colour.petal are valid
      if (!is.null(params$colour.petal)) {
        if (length(params$colour.petal) != length(cols))
          stop('The number of colours specified in',
               '"colour.petal" are not equal to the number',
               'of variables specified in "cols".')

        if (!all(iscolour(params$colour.petal))) {
          stop('Invalid colour(s) specified in "colour.petal".')
        }
    ...
    draw_panel = function(data, panel_params,
      ...
      gdata <- NULL
      if (is.null(fill.petal) & !is.null(fill.gradient)) {
        gdata <- data[, cols]
    ...
    draw_key = function(data, params, size) {
      ...
      flowerglyphGrob(
        ...
        size = data$size,
        petal.width = params$petal.width,
        petal.base.shape = params$petal.base.shape,
        petal.tip.shape = params$petal.tip.shape,
        petal.tip.notch = params$petal.tip.notch,
        petal.tip.notch.width = params$petal.tip.notch.width,
        petal.base.notch = params$petal.base.notch,
        petal.base.notch.width = params$petal.base.notch.width,
        petal.waist = params$petal.waist,
        petal.waist.position = params$petal.waist.position,
        petal.waist.width = params$petal.waist.width,
        petal.curvature = params$petal.curvature,
        petal.curvature.position = params$petal.curvature.position,
        petal.curvature.width = params$petal.curvature.width,
        edges = params$edges,
        col = if (is.null(params$colour.petal)) {
          data$colour
        } else {
          params$colour.petal
        },
        fill = if (is.null(params$fill.petal)) {
          data$fill
        } else {
          params$fill.petal
        },
        ...
        centre.size = data$centre.size,
        col.centre = if (is.null(params$colour.centre)) {
          data$colour
        } else {
          params$colour.centre
        },
        fill.centre = if (is.null(params$fill.centre)) {
          data$fill
        } else {
          params$fill.centre
        },
...
#' grid::makeContent function for the grobTree of flowerglyphGrob objects
#' @param x A grid grobTree.
#' @export
#' @noRd
makeContent.flowerglyphtree <- function(x) {
  ...
  if (g$repel) {
    ...
    glorg <- lapply(seq_along(g$data$x),
                    function(i) {
                      flowerglyphGrob(x = g$data$x[i],
                                      y = g$data$y[i],
                                      z = unlist(g$data[i, g$cols]),
                                      size = g$data$size[i],
                                      petal.width = g$petal.width,
                                      petal.base.shape = g$petal.base.shape,
                                      petal.tip.shape = g$petal.tip.shape,
                                      petal.tip.notch = g$petal.tip.notch,
                                      petal.tip.notch.width =
                                        g$petal.tip.notch.width,
                                      petal.base.notch = g$petal.base.notch,
                                      petal.base.notch.width =
                                        g$petal.base.notch.width,
                                      petal.waist = g$petal.waist,
                                      petal.waist.position =
                                        g$petal.waist.position,
                                      petal.waist.width = g$petal.waist.width,
                                      petal.curvature = g$petal.curvature,
                                      petal.curvature.position =
                                        g$petal.curvature.position,
                                      petal.curvature.width =
                                        g$petal.curvature.width,
                                      edges = g$edges,
                                      col = "gray",
  ...
  gl <- lapply(seq_along(g$data$x),
               function(i) {
                 ...
                 petal.tip.shape = g$petal.tip.shape,
                 petal.tip.notch = g$petal.tip.notch,
                 petal.tip.notch.width = g$petal.tip.notch.width,
                 petal.base.notch = g$petal.base.notch,
                 petal.base.notch.width = g$petal.base.notch.width,
                 petal.waist = g$petal.waist,
                 petal.waist.position = g$petal.waist.position,
                 petal.waist.width = g$petal.waist.width,
                 petal.curvature = g$petal.curvature,
                 petal.curvature.position = g$petal.curvature.position,
                 petal.curvature.width = g$petal.curvature.width,
                 edges = g$edges,
                 col = if (is.null(g$colour.petal)) {
                   g$data$colour[i]
                 } else {
                   g$colour.petal
                 },
                 fill = if (is.null(g$fill.petal)) {
                 ...
                 scale.area = g$scale.area,
                 centre = g$centre,
                 centre.size = g$data$centre.size[i],
                 col.centre = if (is.null(g$colour.centre)) {
                   g$data$colour[i]
                 } else {
                   g$colour.centre
                 },
                 fill.centre = if (is.null(g$fill.centre)) {
                   g$data$fill[i]
                 } else {
                   g$fill.centre
                 },
